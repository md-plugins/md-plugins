...
::: warning
Q-Press is for Quasar Vite projects using `@quasar/app-vite` `>=3.0.0-rc.3` at this time. TypeScript processing is also required. Do not use if you are using Webpack or have a JavaScript-only project.
:::
...
**A.** Q-Press `0.1.0-rc.18` targets Quasar CLI Vite 3, so browser-side code must use `import.meta.env` instead of `process.env`.
